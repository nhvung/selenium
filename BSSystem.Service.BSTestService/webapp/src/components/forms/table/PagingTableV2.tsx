                ...
                bodyElement = (<>
                    {Array.from<rowProps>(totalRef.records).map((record, rowIdx) => {
                        if (record.cells?.length > 0) {
                            ...
                            >
                                {props.includeCheckbox ? <td className={`${thisCss["PagingTable-td-chk"]}`} key={cellKey} align='center'><input id={`${rowId}-chk`} name={thisId} className={`${thisCss["PagingTable-chkrow"]}`} defaultValue={rowValue} type="checkbox" onChange={props.checkRowOnClick ? undefined : chkRowChange} /></td> : undefined}
